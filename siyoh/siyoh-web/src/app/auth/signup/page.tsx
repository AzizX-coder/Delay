...
export default function SignupPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setErr(null);
    if (!username.match(/^[a-z0-9._]{3,24}$/)) {
      setErr('Username: 3-24 chars, lowercase letters, numbers, dot, underscore.');
      return;
    }
  ...
  return (
    <div style={{ ...liquidSurface({ dark, intensity: 'med' }), borderRadius: 22, padding: 28 }}>
      <h1 style={{ fontFamily: 'var(--font-newsreader)', fontSize: 30, fontWeight: 500, color: ink,
        letterSpacing: -0.6, margin: '0 0 6px' }}>Begin your story.</h1>
      <p style={{ fontFamily: 'var(--font-newsreader)', fontSize: 15, color: mute, margin: '0 0 22px' }}>
        Free, forever. Read or write — you decide.
      </p>
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <input required placeholder="username (e.g. amara.writes)" value={username}
      ...
      <p style={{ marginTop: 14, fontFamily: 'var(--font-geist)', fontSize: 11.5,
        color: mute, textAlign: 'center', lineHeight: 1.5 }}>
        By creating an account you agree to our terms and privacy.
      </p>
      <div style={{ marginTop: 16, textAlign: 'center', fontFamily: 'var(--font-geist)', fontSize: 13, color: mute }}>
        Already have one? <Link href="/auth/login" style={{ color: tokens.orange, fontWeight: 500, textDecoration: 'none' }}>Sign in</Link>
      </div>
    </div>
  );
}
