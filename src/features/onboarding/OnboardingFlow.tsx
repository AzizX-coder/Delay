            ...
            {step === 0 && (
              ...
              >
                <div className="flex items-center gap-4 mb-8">
                  <div className="p-3 rounded-2xl bg-accent shadow-xl shadow-accent/20">
                    <Logo size={40} />
                  </div>
                  <div>
                    ...
                    <p className="text-[14px] text-text-tertiary font-medium">
                      Select your interface style to begin
                    </p>
                  </div>
                </div>
                  ...
                  {[
                    ...
                    <motion.button
                      ...
                      className={`flex flex-col items-center justify-center p-6 rounded-3xl border-2 transition-all cursor-pointer relative
                        ${theme === opt.value
                            ? "border-accent bg-accent/5 text-accent shadow-xl shadow-accent/5"
                            : "border-border/40 bg-bg-secondary/20 text-text-secondary hover:border-border"
                        }`}
                    >
                      <div className={`mb-4 p-3 rounded-2xl ${theme === opt.value ? "bg-accent text-white shadow-lg" : "bg-bg-hover text-text-tertiary"}`}>
                        {opt.icon}
                      </div>
                      ...
                      {theme === opt.value && (
                        <motion.div layoutId="check" className="absolute top-3 right-3 text-accent">
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-2 overflow-y-auto max-h-[320px] pr-2 custom-scrollbar">
                  {LANGUAGES.map((lang) => (
                    <motion.button
                      key={lang.code}
                      whileHover={{ x: 4, backgroundColor: "rgba(var(--color-accent-rgb), 0.05)" }}
            ...
            {step === 2 && (
              ...
              >
                ...
                <div className={`p-5 rounded-[24px] mb-6 flex items-center gap-4 border transition-all
                  ...
                  <div>
                    ...
                    <p className="text-[12px] opacity-80 font-medium">
                      {ollamaOnline ? "Local models are ready for agentic execution." : "Local modes won't work, but Cloud mode is still available."}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 overflow-y-auto max-h-[220px] pr-2 custom-scrollbar">
                  <ModelItem 
                    name="glm-5:cloud" 
                    desc="High-speed, premium performance (No setup)" 
